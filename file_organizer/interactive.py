...
def configure_from_prompt(args: Namespace) -> bool:
    ...
    print("Example: organize my Downloads with AI, create folders if needed, then move files")
    request = input("> ").strip()
    if not request:
        return False
    ...
    args.ai_scope = args.ai_scope or ("all" if _has_any(lower, ["all files", "every file", "everything", "move all"]) else "smart")
...
def _print_interactive_summary(args: Namespace) -> None:
    ...
    print(f"- output: {args.output or 'target/Organized'}")
    print(f"- mode: {'apply/move files' if args.apply else 'dry run/preview'}")
    print(f"- use_ai: {args.use_ai}")
    if args.use_ai:
        print(f"- ai_provider: {args.ai_provider or 'from .env/default'}")
        print(f"- ai_scope: {args.ai_scope or 'smart'}")
        print(f"- ai_prefer: {args.ai_prefer}")
        print(f"- ai_custom_folders: {args.ai_custom_folders}")
    if args.auto_apply_min_confidence is not None:
        print(f"- auto_apply_min_confidence: {args.auto_apply_min_confidence:.2f}")
    print()
